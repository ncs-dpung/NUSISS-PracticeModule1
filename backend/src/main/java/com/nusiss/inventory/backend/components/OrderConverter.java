...
public class OrderConverter {
  ...
  public OrderDto convertOrderToDto(Order order) {
    ...
    Set<OrderItemDto> itemDtos =
        order.getItems().stream()
            .map(
                item -> {
                  String productName =
                      productIdToNameMap.getOrDefault(
                          item.getProduct().getId(), "Product Name Not Found");
                  return new OrderItemDto(
                      item.getProduct().getId(), productName, item.getQuantity(), item.getPrice());
                })
            .collect(Collectors.toSet());
    ...
    OrderDto dto =
        new OrderDto(
            ...
            order.getStaff().getFirstName(),
            order.getStaff().getLastName(),
