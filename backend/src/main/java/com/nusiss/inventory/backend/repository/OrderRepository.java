  ...
  @Query(
      value =
          "SELECT YEAR(o.date_placed) AS year, MONTH(o.date_placed) AS month, COUNT(*) AS"
              + " totalOrders, SUM(oi.price) AS totalRevenue FROM tbl_order o INNER JOIN"
              + " tbl_order_items oi ON o.order_id = oi.order_id GROUP BY YEAR(o.date_placed),"
              + " MONTH(o.date_placed) ORDER BY YEAR(o.date_placed), MONTH(o.date_placed)",
      nativeQuery = true)
