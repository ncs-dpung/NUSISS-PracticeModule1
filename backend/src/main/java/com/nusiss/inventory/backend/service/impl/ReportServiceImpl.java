...
public class ReportServiceImpl implements ReportService {
  ...
  public MonthlyReportDto getMonthlyReport(int year, int month) {
    ...
    List<Object[]> monthlySalesData =
        orderRepository.findTotalSalesAndRevenueForSpecificMonth(year, month);
    TotalSalesDto totalSalesDto =
        monthlySalesData.stream()
            .map(
                data ->
                    new TotalSalesDto(
                        year,
                        ...
                        ""))
            .findFirst()
            .orElse(new TotalSalesDto(year, month, 0L, BigDecimal.ZERO, ""));
    ...
    List<CategoryRevenueDto> revenueByCategory =
        categoryRepository.findRevenueByCategory(year, month);
  ...
  private List<TopSellingProductDto> getTopSellingProducts(List<Object[]> rawResults) {
    return rawResults.stream()
        .map(data -> new TopSellingProductDto((String) data[0], ((Number) data[1]).longValue()))
        .toList();
  }
}
