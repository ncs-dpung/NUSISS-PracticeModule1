...
public class OrderServiceImplTest {
    ...
    @Mock
    private Map<String, OrderUpdateStrategy> updateStrategies;
    @Mock
    ...
    public void setUp() {
        ...
        orderService = new OrderServiceImpl(orderConverter, orderRepository, orderStatusRepository, productRepository, updateStrategies, observers);
    }
    ...
    public void testUpdateOrderStatus_Processing() {
        // Arrange
        Long orderId = 1L;
        StatusUpdateRequest statusUpdateRequest = new StatusUpdateRequest(new OrderStatus(2L, "Delivered"), new Date());
        Order order = new Order();
        order.setOrderId(orderId);
        order.setStatus(new OrderStatus(1L, "Processing"));
        when(orderRepository.findById(orderId)).thenReturn(Optional.of(order));
        when(orderStatusRepository.findById(anyLong())).thenReturn(Optional.of(new OrderStatus(2L, "Delivered")));
        when(orderRepository.save(any(Order.class))).thenReturn(order);
    ...
    void testUpdateOrderStatus_Delivered() {
        ...
        when(orderRepository.findById(orderId)).thenReturn(Optional.of(order));
        when(orderStatusRepository.findById(deliveredStatus.getId())).thenReturn(Optional.of(deliveredStatus));
        when(orderRepository.save(any(Order.class))).thenReturn(order);
        when(orderConverter.convertOrderToDto(order)).thenReturn(new OrderDto());
    ...
    void testUpdateOrderStatus_NotFound() {
        ...
        assertThrows(EntityNotFoundException.class, () -> orderService.updateOrderStatus(orderId, statusUpdateRequest));
    }
