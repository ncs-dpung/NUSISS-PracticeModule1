...
class ReportServiceImplTest {
  ...
  void getMonthlyReportShouldReturnCorrectData() {
    ...
    when(orderRepository.findMostSoldProductForMonthAndYear(
            String.valueOf(year), String.valueOf(month)))
        .thenReturn(
            List.of(
                new Object[] {mostSoldProductName, totalQuantitySold},
                new Object[] {mostSoldProductName, totalQuantitySold}));
    ...
    when(categoryRepository.findRevenueByCategory(year, month))
        .thenReturn(
            Arrays.asList(
                new CategoryRevenueDto("Mock Category 1", new BigDecimal("500")),
